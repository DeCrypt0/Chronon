...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CHNMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CHNMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CHNMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chronon-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("CHNMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running CHNMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCHNMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCHNMiner() exception");
    }

    LogPrintf("ThreadCHNMiner exiting\n");
}
