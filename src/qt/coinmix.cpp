...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::CHN)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeCHNAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCHNAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeCHNAmount = strAnonymizeCHNAmount.remove(strAnonymizeCHNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCHNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCHNAmount * COIN) nMaxToAnonymize = nAnonymizeCHNAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCHNAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeCHNAmount));
        strAnonymizeCHNAmount = strAnonymizeCHNAmount.remove(strAnonymizeCHNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCHNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
